...
//实现类库的加载和二维码的生成
function phpqrcode($url,$path){
	if(!vendor('phpqrcode.phpqrcode')){
		die('载入 phpqrcode 类库失败！');
	}
	return QRcode::png($url, $path, null, 6, 0);
}

//根据ID计算保存路径
function genIdPath($id){
	if(strlen($id)>9) return $id;
	$id = str_pad($id,9,'0',STR_PAD_LEFT);
	$new_id = substr($id,0,3);
	for($i=3;$i<9;++$i){
		$new_id .= ($i%2==0) ? $id[$i] : '/'.$id[$i] ;
	}
	return $new_id;
}
